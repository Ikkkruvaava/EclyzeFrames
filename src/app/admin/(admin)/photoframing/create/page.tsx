...
export default function CreateFramePage() {
  ...
  const { addFrame } = useFrameStore() as any;
  ...
  const defaultSizes = {
    image: { width: 0.5, height: 0.5, x: 0.25, y: 0.25 },
    text: { width: 0.5, height: 0.1, x: 0.25, y: 0.8 },
  };
  ...
  useEffect(() => {
    ...
    userImg.src = "/api/placeholder/400/400"; // Simulation
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith("image/")) {
      setErrors({ ...errors, frameImage: "Please upload an image" });
      return;
    }
    ...
    img.onload = () => {
      ...
      setEditorData(prev => ({
        ...prev,
        dimensions,
        placementCoords: prev.hasImageArea ? {
          x: Math.round(dimensions.width * defaultSizes.image.x),
          y: Math.round(dimensions.height * defaultSizes.image.y),
          width: Math.round(dimensions.width * defaultSizes.image.width),
          height: Math.round(dimensions.height * defaultSizes.image.height),
        } : null,
        textSettings: prev.textSettings.map(ts => ({
          ...ts,
          x: Math.round(dimensions.width * defaultSizes.text.x),
          y: Math.round(dimensions.height * defaultSizes.text.y),
          width: Math.round(dimensions.width * defaultSizes.text.width),
          height: Math.round(dimensions.height * defaultSizes.text.height),
        }))
      }));
      setFrameImage(file);
  ...
  const addTextSettings = () => {
    setEditorData(prev => ({
      ...prev,
      textSettings: [...prev.textSettings, {
        x: 100, y: 100, width: 200, height: 50, font: "Arial", size: 24, color: "#ffffff", align: "center", label: `Text Field ${prev.textSettings.length + 1}`
      }]
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !frameImage) {
      setErrors({ name: !name ? "Name is required" : undefined, frameImage: !frameImage ? "Image is required" : undefined });
      return;
    }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10 pb-6 border-b border-gray-100 dark:border-gray-800">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white mb-2 tracking-tight">Create New Frame</h1>
          <p className="text-gray-500 dark:text-gray-400">Design your dynamic frame layout with text and image zones.</p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
        <aside className="xl:col-span-4 space-y-8 order-2 xl:order-1">
          {/* Metadata */}
          <section className="bg-white dark:bg-gray-900 p-6 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-xl shadow-gray-200/40 dark:shadow-none space-y-5">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border-none rounded-2xl ring-1 ring-gray-200 dark:ring-gray-700 focus:ring-2 focus:ring-blue-500 transition-all outline-none"
                placeholder="e.g. Wedding Floral Frame"
              />
              {errors.name && <p className="text-red-500 text-xs mt-2 ml-1 font-medium">{errors.name}</p>}
            </div>

            <div>
              <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2 ml-1">Frame Asset</label>
              <div
                onClick={() => fileInputRef.current?.click()}
                className={`relative cursor-pointer group rounded-2xl border-2 border-dashed transition-all duration-300 p-8 flex flex-col items-center justify-center gap-3 
                  ${previewImage ? "border-green-500/50 bg-green-50/50 dark:bg-green-900/10" : "border-gray-200 dark:border-gray-700 hover:border-blue-500/50 hover:bg-blue-50/30"}`}
              >
                <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleImageUpload} />
                {previewImage ? (
                  <div className="flex flex-col items-center">
                    <div className="relative w-24 h-24 rounded-lg overflow-hidden border border-green-200 mb-3 shadow-lg">
                      <NextImage src={previewImage} fill className="object-cover" alt="Preview" />
                    </div>
                    <span className="text-xs font-bold text-green-700 dark:text-green-400 flex items-center gap-1"><CheckCircle2 size={12} /> File Ready</span>
                  </div>
                ) : (
                  <>
                    <div className="p-4 bg-gray-100 dark:bg-gray-800 rounded-2xl group-hover:scale-110 transition-transform duration-300"><Upload size={24} className="text-gray-400 group-hover:text-blue-500" /></div>
                    <span className="text-sm font-semibold text-gray-500">Tap to upload PNG/JPG</span>
                  </>
                )}
              </div>
              {errors.frameImage && <p className="text-red-500 text-xs mt-2 ml-1 font-medium">{errors.frameImage}</p>}
            </div>
          </section>

          {/* Controls */}
          <section className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-xl shadow-gray-200/40 dark:shadow-none overflow-hidden">
              ...
              {(["image", "text"] as const).map(tab => (
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-4 rounded-2xl font-bold text-sm transition-all duration-300
                    ${activeTab === tab ? "bg-blue-600 text-white shadow-lg shadow-blue-500/30" : "text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800"}`}
                >
                  {tab === "image" ? <Layers size={18} /> : <Type size={18} />}
                  <span className="capitalize">{tab} Editor</span>
                </button>
              ))}
            </div>
              ...
              {activeTab === "image" && (
                <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
                  <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-2xl">
                    <span className="text-sm font-bold text-gray-700 dark:text-gray-200">Enable Image Slot</span>
                    <button
                      ...
                      {["x", "y", "width", "height"].map(prop => (
                        <div key={prop} className="space-y-1">
                          ...
                          <input
                            ...
                            className="w-full px-3 py-2.5 bg-gray-50 dark:bg-gray-800 rounded-xl text-sm font-bold border-none ring-1 ring-gray-100 dark:ring-gray-700 outline-none"
                          />
                        </div>
                      ))}
                    </div>
              ...
              {activeTab === "text" && (
                <div className="space-y-6 animate-in slide-in-from-left-4 duration-300">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-bold text-gray-900 dark:text-white">Layers ({editorData.textSettings.length})</h3>
                    <button onClick={addTextSettings} className="p-2 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-xl hover:scale-105 transition-transform"><Plus size={18} /></button>
                  </div>
                  ...
                  <div className="space-y-3 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                    {editorData.textSettings.map((_, i) => (
                      <div
                        ...
                        className={`p-3 rounded-2xl flex items-center justify-between cursor-pointer transition-all border-2 
                           ${activeTextIndex === i ? "border-blue-500 bg-blue-50/50 dark:bg-blue-900/20" : "border-transparent bg-gray-50 dark:bg-gray-800 opacity-60 hover:opacity-100"}`}
                      >
                        <span className="text-sm font-bold">Text Layer {i + 1}</span>
                        <button onClick={(e) => { e.stopPropagation(); removeTextSettings(i); }} className="text-gray-400 hover:text-red-500 transition-colors"><Trash2 size={16} /></button>
                      </div>
                    ))}
                  </div>
          ...
          <button
            ...
            className={`w-full py-4 rounded-3xl font-black text-lg flex items-center justify-center gap-3 transition-all transform active:scale-95
            ${isSubmitting || !previewImage ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-blue-600 text-white shadow-2xl shadow-blue-500/30 hover:-translate-y-1"}`}
          >
            {isSubmitting ? <Loader2 className="animate-spin" /> : <Save size={20} />}
            {isSubmitting ? "Processing..." : "Capture & Create"}
          </button>
        </aside>

        <main className="xl:col-span-8 space-y-6 order-1 xl:order-2">
          <div className="sticky top-6 z-30 flex items-center justify-between p-4 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border border-white dark:border-gray-800 rounded-3xl shadow-2xl shadow-gray-200/20 dark:shadow-none">
            <div className="flex gap-2">
              <button onClick={() => setShowGrid(!showGrid)} className={`p-2.5 rounded-2xl transition-all ${showGrid ? "bg-blue-100 text-blue-600 shadow-inner" : "bg-gray-50 text-gray-400 dark:bg-gray-800"}`} title="Toggle Grid"><Grid size={18} /></button>
              <button onClick={() => setZoom(prev => Math.min(prev + 0.1, 2))} className="p-2.5 rounded-2xl bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-100"><ZoomIn size={18} /></button>
              <button onClick={() => setZoom(prev => Math.max(prev - 0.1, 0.5))} className="p-2.5 rounded-2xl bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-100"><ZoomOut size={18} /></button>
              <button onClick={() => setZoom(1)} className="p-2.5 rounded-2xl bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-100"><RotateCcw size={18} /></button>
            </div>
            ...
            <div className="flex items-center gap-4">
              <input
                ...
                className="max-w-[120px] md:max-w-[200px] px-4 py-2 bg-gray-50 dark:bg-gray-800 border-none rounded-2xl text-xs font-bold outline-none ring-1 ring-gray-100 dark:ring-gray-700 focus:ring-blue-500"
              />
              <button
                onClick={() => setPreviewMode(!previewMode)}
                className={`px-5 py-2.5 rounded-2xl font-bold text-xs flex items-center gap-2 transition-all 
                    ${previewMode ? "bg-amber-100 text-amber-600 shadow-inner" : "bg-gray-50 text-gray-600 dark:bg-gray-800 dark:text-gray-300"}`}
              >
          ...
          <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-8 rounded-[2rem] text-white shadow-2xl relative overflow-hidden group">
            <div className="relative z-10">
              <h3 className="text-xl font-black mb-3 flex items-center gap-2 italic"><Info size={20} /> Pro Design Tips</h3>
              <div className="grid md:grid-cols-2 gap-6 opacity-90">
                <div className="bg-white/10 backdrop-blur-md p-4 rounded-2xl border border-white/20">
                  <strong className="block text-sm mb-1 uppercase tracking-wider font-extrabold">Layer Ordering</strong>
                  <p className="text-xs leading-relaxed">We render elements in the order they appear in your layers list. Dragging to reorder is coming soon!</p>
                </div>
                <div className="bg-white/10 backdrop-blur-md p-4 rounded-2xl border border-white/20">
                  <strong className="block text-sm mb-1 uppercase tracking-wider font-extrabold">Responsive Sizing</strong>
                  <p className="text-xs leading-relaxed">The canvas respects your asset&apos;s natural aspect ratio. Use the dimension inputs to override manually.</p>
                </div>
              </div>
            </div>
            <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:rotate-12 transition-transform duration-700"><Save size={120} /></div>
          </div>
        </main>
      </div>
